...
for token in ['chopper-native-mp3-v1-cache-6-telephony-wav','CACHE_PREFIX','k.startsWith(CACHE_PREFIX)','cache:"reload"','ASSET_URLS','!ASSET_URLS.has(url.href)','ACTIVATE_UPDATE','e.request.mode==="navigate"','cache:"no-store"','./js/app.bundle.js']:
...
if pkg.get('version')!='1.0.5': print('FAIL package version');sys.exit(1)
...
print('PASS NativeMP3Converter V1.0.5 classic-bundle / telephony-WAV / same-folder UX / picker-ID / TTL / output markers')
